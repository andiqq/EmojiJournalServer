...
function killServerProcesses() {
    pid="$(pgrep EmojiJournalServerScratch)"
    if [ "$pid" != "" ]
    then
        echo "killing EmojiJournalServerScratch"
        kill $pid
    fi
